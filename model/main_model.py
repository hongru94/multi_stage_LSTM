...
params = OrderedDict(
    ...
    num_epochs = [50],
    dropout_rate = [0.8],
    lossfunc = [nn.SmoothL1Loss(beta=0.1, reduction = 'sum')]
)


#data_file =
#model_file = '/Users/hongru/Projects/Covid_Forecast/Paper_revision/results_with_genomic_2/main/'
...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_layer_size, num_layers, output_size, batch_size, dropout_rate):
        super().__init__()
...
for run in RunBuilder.get_runs(params):
    ...
    # df = df[~df.index.get_level_values('FIPS').isin(['12'])]
    ...
    full_data_main, state_ordered = prepare_data_main_model(select_df, run.seq_length,
                                                                run.output_size, run.date_range)

    model_main = LSTM(run.input_size, run.hidden_layer_size, run.num_layers, run.output_size, run.batch_size,
                          run.dropout_rate)
